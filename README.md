...
> 2026-09-11 更新：流程图已改为“阶段语义 + 用户决策”视图，并完成第二轮几何优化。框内只保留用户能理解的流程说明，不再写入具体 Python 脚本名；图片验证分支明确区分“保留整图”和“切分并交叉验证”；节点比例、边框和颜色已统一，最终交付明确为 Markdown + PDF。
...
主干只有一条：**1.0 环境自检 → 2.0 转 Word → 2.1 抽正文和图 → 2.2 同时询问 Word 满意度与是否交叉验证图片 → 3.0（必要时回退）→ 4.0 按选择切面板或保留整图 → 5.0 翻译 → 5.1 写 Markdown → 5.2 图归位 → 5.3 加目录 → 5.4 图片内嵌 → 6.0 转 PDF → 7.0 自检**。切面板不再是强制步骤。
...
> Word 文档只是脚手架，**不是交付物**。最终只输出 Markdown + PDF；图片先放在临时目录，5.4 会内嵌进 Markdown，之后不交付切图合集。
...
Claude 会自动完成：**1.0 环境自检** → 2.0 转 Word → 2.1 抽正文与图 → **2.2 问你 Word 是否满意、是否交叉验证图片** → 按选择执行 3.0/4.0 → 5.0 分批翻译 → 5.1 写 Markdown → 5.2 图归位 → 5.3 加目录 → 5.4 图片内嵌 → 6.0 转 PDF → 7.0 自检并清理临时文件。
...
5.2 归位完成后运行。脚本把本地 `![...](路径)` 改成 `![...](data:image/...;base64,...)`，保留原来的正文位置和图注，并留下 `.bak`。成功后 Markdown 不再依赖同级图片目录；确认 5.4 和 6.0 都成功后，再删除临时的 `panels/`、`_figs/`、`media/` 及本次流程产生的 `.bak`。退出码 `3` 表示有缺失图片或远程图片，不能直接交付。
